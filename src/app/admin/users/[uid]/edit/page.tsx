            ...
            <div style={{ flex: 1, padding: '32px', overflowY: 'auto' }}>
                <div style={{ maxWidth: 800, margin: '0 auto' }}>
                    <PageHero 
                        ...
                        subtitle="Actualiza la información y rol del usuario"
                        parentTitle="Usuarios"
                        parentHref={`/admin/users/${uid}`}
                    />
                    ...
                    <form onSubmit={handleSubmit} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 32, padding: 40, boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
                        <div style={{ display: 'grid', gap: 32 }}>
                            ...
                            <div>
                                <h3 style={{ margin: '0 0 24px 0', fontSize: 16, fontWeight: 800, color: 'var(--foreground)', display: 'flex', alignItems: 'center', gap: 10 }}>
                                    <Shield size={20} style={{ color: 'var(--brand)' }} /> Roles y Permisos
                                </h3>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                                    <div>
                                            ...
                                            {(['USUARIO', 'USUARIO_SST', 'USUARIO_PROFESIONAL', 'INSTRUCTOR', 'ADMIN', 'SUPER_ADMIN'] as const).map((role) => (
                                                <button
                                                    ...
                                                    style={{
                                                        ...
                                                        border: '1px solid var(--border)', background: formData.role === role ? 'var(--accent)' : 'var(--text-on-brand)',
                                                        color: formData.role === role ? 'var(--brand)' : 'var(--text-secondary)',
                                    ...
                                    <div>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                            <button
                                                ...
                                                style={{
                                                    padding: '12px 16px', borderRadius: 12,
                                                    border: '1px solid var(--border)', background: formData.status === 'ACTIVE' ? '#DCFCE7' : 'var(--text-on-brand)',
                                                    color: formData.status === 'ACTIVE' ? '#166534' : 'var(--text-secondary)',
                                                    fontWeight: 700, fontSize: 13, cursor: 'pointer', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 8,
                                                }}
                                            >
                                            ...
                                            <button
                                                ...
                                                style={{
                                                    padding: '12px 16px', borderRadius: 12,
                                                    border: '1px solid var(--border)', background: formData.status === 'PENDING' ? '#FEF3C7' : 'var(--text-on-brand)',
