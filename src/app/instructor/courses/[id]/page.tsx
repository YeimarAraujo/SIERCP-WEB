            ...
            <div style={{ flex: 1, padding: '24px 32px', overflowY: 'auto' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20, marginBottom: 40 }}>
                    {[
                        ...
                        { label: 'Estructura', value: guides.length, sub: 'Módulos cargados', icon: BookOpen, color: 'var(--clr-accent)' },
                            ...
                            <div style={{ display: 'grid', gap: 14 }}>
                                {guides.length === 0 ? (
                                    <div style={{ padding: 80, textAlign: 'center', background: 'var(--card)', borderRadius: 24, border: '1px solid var(--border)' }}>
                                        ...
                                        <p style={{ color: 'var(--text-secondary)', fontWeight: 500 }}>No hay contenido teórico cargado para este curso.</p>
                                    </div>
                                ) : (
                                    guides.map(g => (
                                        <div key={g.id} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 20, padding: '20px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                            ...
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                                <button onClick={() => window.open(g.contentUrl, '_blank')} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: '10px', color: 'var(--text-secondary)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }} title="Ver PDF">
                                                    <FileText size={18} /> <span style={{ fontSize: 13, fontWeight: 700 }}>Ver Material</span>
                                                </button>
                                                <div style={{ width: 1, height: 24, background: 'var(--border)', margin: '0 8px' }} />
                                                <Link href={`/instructor/courses/${id}/guides/${g.id}/edit`} style={{ color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', padding: 8 }}>
                                                    <Settings size={20} />
                                                </Link>
                    ...
                    {activeTab === 'config' && (
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 32 }}>
                            <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 24, padding: 32 }}>
                                ...
                                <p style={{ color: 'var(--text-secondary)', fontSize: 14, lineHeight: 1.6, marginTop: 12 }}>Configura los parámetros de evaluación, códigos de acceso y visibilidad del curso.</p>
                                <div style={{ marginTop: 32, display: 'grid', gap: 12 }}>
                                    <button style={{ width: '100%', padding: '14px', borderRadius: 12, background: 'var(--brand)', color: 'var(--text-on-brand)', border: 'none', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>Editar Información</button>
                                    <button style={{ width: '100%', padding: '14px', borderRadius: 12, background: 'var(--muted)', color: '#EF4444', border: '1px solid #FEE2E2', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>Archivar Curso</button>
                                </div>
                            </div>
                            <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 24, padding: 32 }}>
