    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--muted)' }}>
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 24, justifyContent: 'flex-end' }}>
                <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: '10px 20px', display: 'flex', alignItems: 'center', gap: 12, boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                    <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#10B981', boxShadow: '0 0 10px rgba(16, 185, 129, 0.5)' }} />
                    <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-secondary)' }}>{activeSessions.length} SESIONES ACTIVAS</span>
                </div>
            </div>
                ...
                {loading && activeSessions.length === 0 ? (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
                        {[1, 2, 3].map(i => (
                            <div key={i} style={{ height: 260, background: 'var(--card)', borderRadius: 24, border: '1px solid var(--border)', animation: 'pulse 2s infinite' }} />
                        ))}
                    </div>
                ) : activeSessions.length === 0 ? (
                    <div style={{ background: 'var(--card)', border: '1px dashed var(--border)', borderRadius: 32, padding: '120px 0', textAlign: 'center' }}>
                        <div style={{ width: 80, height: 80, borderRadius: 40, background: 'var(--muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px auto', color: 'var(--border-strong)' }}>
                            <Monitor size={40} />
                        </div>
                        <h3 style={{ fontSize: 20, fontWeight: 800, color: 'var(--foreground)', marginBottom: 8 }}>Sin Transmisiones Entrantes</h3>
                        <p style={{ color: 'var(--text-secondary)', fontSize: 15, maxWidth: 400, margin: '0 auto' }}>Inicia una sesión en el simulador SIERCP para comenzar a recibir datos de telemetría en tiempo real.</p>
                    </div>
                ) : (
                            ...
                            <div key={s.id} style={{ 
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                                        <div style={{ width: 44, height: 44, borderRadius: 14, background: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--brand)', border: '1px solid rgba(24, 0, 173, 0.1)' }}>
                                            <User size={22} />
                                        </div>
                                        <div>
                                            <div style={{ fontWeight: 800, color: 'var(--foreground)', fontSize: 16 }}>{s.studentName}</div>
                                            <div style={{ fontSize: 12, color: 'var(--text-secondary)', fontWeight: 600 }}>{s.scenarioTitle}</div>
                                        </div>
                                    </div>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                                    ...
                                    <MetricBox label="Profundidad" value={`${(s.metrics?.averageDepthMm ?? 0).toFixed(0)}mm`} color="var(--foreground)" icon={Zap} />
                                    <MetricBox label="Frecuencia" value={(s.metrics?.averageRatePerMin ?? 0).toFixed(0)} color="var(--foreground)" icon={Timer} />
                                </div>

                                {/* Visual Telemetry (Simulated Rhythm) */}
                                <div style={{ height: 60, background: 'var(--muted)', borderRadius: 16, overflow: 'hidden', position: 'relative', border: '1px solid var(--muted)' }}>
                                    <div style={{ position: 'absolute', inset: 0, opacity: 0.05, background: 'linear-gradient(90deg, transparent 95%, var(--brand) 100%)', backgroundSize: '40px 100%' }} />
                                    ...
                                    <svg width="100%" height="100%" viewBox="0 0 400 60" style={{ position: 'absolute' }}>
                                        <path d="M0 30 L40 30 L45 20 L55 40 L60 30 L100 30 L105 10 L115 50 L120 30 L160 30 L165 25 L175 35 L180 30 L220 30 L225 5 L235 55 L240 30 L280 30 L285 20 L295 40 L300 30 L340 30 L345 10 L355 50 L360 30 L400 30" 
                                              fill="none" stroke="var(--brand)" strokeWidth="2" strokeOpacity="0.8" 
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
                                    ...
                                    <button style={{ background: 'var(--accent)', border: 'none', borderRadius: 8, padding: '6px 12px', color: 'var(--brand)', fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
...
function MetricBox({ label, value, color, icon: Icon }: any) {
    return (
        <div style={{ background: 'var(--muted)', borderRadius: 16, padding: '16px 12px', textAlign: 'center', border: '1px solid var(--muted)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, color: 'var(--text-muted)', fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>
                <Icon size={12} /> {label}
            </div>
